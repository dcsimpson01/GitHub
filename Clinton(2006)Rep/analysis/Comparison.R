...
testplot <- ggplot(data = subset(data), aes(x=C_pctsp, y=C_pcti,colour=(x106mean-mean(x106mean))/sd(x106mean))) +
  geom_point(shape=1,size=2,colour="black") +
  geom_point(aes(colour=x106mean)) +
  #geom_bin2d()+
  #stat_summary_2d(data,aes(z = splc), bins = 30, fun = mean)+
  geom_rug()+
  #geom_density_2d()+
...
trial <- as.data.frame(matrix(c(rep(0,432*3*8)),nrow=432*3,ncol=8))
trial[1:432,2] <- c(rep("SameParty",432))
trial[1:432,1] <- data[,c("cdcoden")]
trial[1:432,3:8] <- data[,c("splc","C_pctsp","partyname","lc","x106mean","x106kmean")]
trial[433:864,2] <- c(rep("Independent",432))
trial[433:864,1] <- data[,c("cdcoden")]
trial[433:864,3:8] <- data[,c("ilc","C_pcti","partyname","lc","x106mean","x106kmean")]
trial[865:1296,2] <- c(rep("OppositeParty",432))
trial[865:1296,1] <- data[,c("cdcoden")]
trial[865:1296,3:8] <- data[,c("oplc","C_pctop","partyname","lc","x106mean","x106kmean")]

ggplot(data = trial) + geom_bar(aes(y = V4, x = V1, fill = V2),
                    stat="identity")


ggplot(data= subset(trial,V5=="Democrat"), aes(x = factor(V1), y = V4, fill = V3)) +
